...
class UserLoginApiView(APIView):
    def post(self, request):
        ...
        if serializer.is_valid():
            username = serializer.validated_data['username'] #username ke niye asbo
            password = serializer.validated_data['password'] #password ke niye asbo

            user = authenticate(username= username, password=password)
            
            if user:
                #token model theke check kore dekbo ei user er kono token amder DB te ase kina
